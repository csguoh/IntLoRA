...
from utils.quant_utils import batch_mse,batch_max
...
class UniformAffineQuantizer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        if self.inited is False:
            if self.leaf_param:
                delta, zero_point = self.init_quantization_scale(x.detach(), self.channel_wise)
    ...
    def init_quantization_scale(self, x: torch.Tensor, channel_wise: bool = False):
        ...
        if channel_wise:
            ...
            if 'max' in self.scale_method:
                delta, zero_point = batch_max(x_clone.view(n_channels, -1), self.sym, 2 ** self.n_bits,
                                              self.always_zero)

            elif 'mse' in self.scale_method:
                delta, zero_point = batch_mse(x_clone.view(n_channels, -1), self.sym, 2 ** self.n_bits,
                                              self.always_zero)
            ...
            if self.leaf_param:  # for momentum update
...
class QuantLayerNormal(nn.Module):
    """
    Quantized Module that can perform quantized convolution or normal convolution.
    To activate quantization, please use set_quant_state function.
    """
    def __init__(self, org_module: Union[nn.Conv2d, nn.Linear, nn.Conv1d], weight_quant_params= None,
                 activation_params= None,):
        super(QuantLayerNormal, self).__init__()
        ...
        self.org_weight = org_module.weight.data
        if org_module.bias is not None:
            self.bias = org_module.bias
            self.org_bias = org_module.bias.data
        else:
            self.bias = None
            self.org_bias = None
        # de-activate the quantized forward default
        self.use_weight_quant = self.weight_quant_params is not None
        self.use_act_quant = self.act_quant_params is not None
    ...
    def forward(self, input: torch.Tensor):
        if self.use_act_quant: # Activation Quant
            input = self.act_quantizer(input)

        if self.use_weight_quant: # Weight Quant
            with torch.no_grad():
